    ...
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const requiredFields = [
                ...
                serieData.genres.length,
                serieData.countries.length,
                serieData.episodes
            ];
            ...
            const hasCastOrCrew = serieData.director.length > 0 || serieData.actors.length > 0 || serieData.author.length > 0;

            if (allFieldsFilled && hasCastOrCrew) {
                series.push(serieData);
                console.log(`✓ готово: "${title}"`);
            } else {
                console.log(`✗ пропущено: "${title}" - не все обязательные поля`);
            }
        }
